//Initialize User character selected 
var characterSelected = false;
//Initialize User defender
var defenderSelected = false;
//Variable to store User selected Jedi
var character = {};
//Variable to store User selected enemy
var defender = {};
//Number of enemies defeated
var enemiesDefeated = 0;
////Boolean to indicate the game status
gameOver = false;

//Character Objects
var darth = {
    name: "Darth Revan",
    health: 120,
    baseAttack: 15,
    attack: 8,
};

var juno = {
    name: "Juno Eclipse",
    health: 100,
    baseAttack: 14,
    attack: 5,
};

var kyle = {
    name: "Kyle Katarn",
    health: 150,
    baseAttack: 20,
    attack: 8,
};

var nym = {
    name: 'Nym',
    health: 180,
    baseAttack: 25,
    attack: 7,
};

var audio = new Audio("assets/music/Theme Song .mp3");
audio.loop = true;
audio.play(); 
//Initialize character value

function initializeCharacter(chosenCharacter) {
    character.name = chosenCharacter.name;
    character.health = chosenCharacter.health;
    character.baseAttack = chosenCharacter.baseAttack;
    character.attack = chosenCharacter.attack;
}

//Initialize defender value

function initializeDefender(chosenDefender) {
    defender.name = chosenDefender.name;
    defender.health = chosenDefender.health;
    defender.baseAttack = chosenDefender.baseAttack;
    defender.attack = chosenDefender.attack;
}

//Move the remaining characters to enemies section
function moveToEnemies() {
    $('.available-character').removeClass("available-character").addClass("enemy-character");
    $("#enemies-available").append($(".enemy-character"));
}

//Reset the game
function resetGame() {
    //Reset all health values to original
    $("#darth-character").children(".health").html(darth.health);
    $("#juno-character").children(".health").html(juno.health);
    $("#kyle-character").children(".health").html(kyle.health);
    $("#nym-character").children(".health").html(nym.health);


    $(".character-image").removeClass("chosen-character enemy-character defender-character").addClass("available-character");
    var available = $(".available-character").show();
    $("#characters-available").html(available);

    $("#game-message").empty();
    $("#restart").hide();

    characterSelected = false;
    defenderSelected = false;
    enemiesDefeated = 0;
    gameOver = false;
    character = {};
    defender = {};
}

$(document).ready(function () {

    // Hide the "Restart" button on document load
    $("#restart").hide();

    // Determine which character the user has clicked
    $("#darth-character").on("click", function () {
        console.log("Darth Raven is selected");

        // User is choosing the character
        if (characterSelected === false) {
            $("#game-message").empty();

            // Set the user's character
            initializeCharacter(darth);
            characterSelected = true;

            // Display the chosen character
            $("#darth-character").removeClass("available-character").addClass("chosen-character");
            $("#chosen-character").append(this);

            // Move the remaining characters to the enemies section
            moveToEnemies();
        } else if ((characterSelected === true) && (defenderSelected === false)) {
            // User is choosing the defender
            if ($("#darth-character").hasClass("enemy-character")) {
                $("#game-message").empty();

                // Set the user's enemy
                initializeDefender(darth);
                defenderSelected = true;

                // Add the character to the defender section
                $("#darth-character").removeClass("enemy-character").addClass("defender-character");
                $("#defender-section").append(this);
            }
        }
    });

    $("#juno-character").on("click", function () {
        console.log("Juno Eclipse is selected");

        // User is choosing the character
        if (characterSelected === false) {
            $("#game-message").empty();

            // Set the user's character
            initializeCharacter(juno);
            characterSelected = true;

            // Display the chosen character
            $("#juno-character").removeClass("available-character").addClass("chosen-character");
            $("#chosen-character").append(this);

            // Move the remaining characters to the enemies section
            moveToEnemies();
        } else if ((characterSelected === true) && (defenderSelected === false)) {
            // User is choosing the defender
            if ($("#juno-character").hasClass("enemy-character")) {
                $("#game-message").empty();

                // Set the user's enemy
                initializeDefender(juno);
                defenderSelected = true;

                // Add the character to the defender section 
                $("#juno-character").removeClass("enemy-character").addClass("defender-character");
                $("#defender-section").append(this);
            }
        }
    });

    $("#kyle-character").on("click", function () {
        console.log("Kyle Katarn is selected");

        // User is choosing the character
        if (characterSelected === false) {
            $("#game-message").empty();

            // Set the user's character
            initializeCharacter(kyle);
            characterSelected = true;

            // Display the chosen character
            $("#kyle-character").removeClass("available-character").addClass("chosen-character");
            $("#chosen-character").append(this);

            // Move the remaining characters to the enemies section
            moveToEnemies();
        } else if ((characterSelected === true) && (defenderSelected === false)) {
            // User is choosing the defender
            if ($("#kyle-character").hasClass("enemy-character")) {
                $("#game-message").empty();

                // Set the user's enemy
                initializeDefender(kyle);
                defenderSelected = true;

                // Add the character to the defender section 
                $("#kyle-character").removeClass("enemy-character").addClass("defender-character");
                $("#defender-section").append(this);
            }
        }
    });

    $("#nym-character").on("click", function () {
        console.log("Nym is selected");

        // User is choosing the character
        if (characterSelected === false) {
            $("#game-message").empty();

            // Set the user's character
            initializeCharacter(nym);
            characterSelected = true;

            // Display the chosen character
            $("#nym-character").removeClass("available-character").addClass("chosen-character");
            $("#chosen-character").append(this);

            // Move the remaining characters to the enemies section
            moveToEnemies();
        } else if ((characterSelected === true) && (defenderSelected === false)) {
            // User is choosing the defender
            if ($("#nym-character").hasClass("enemy-character")) {
                $("#game-message").empty();

                // Set the user's enemy
                initializeDefender(nym);
                defenderSelected = true;

                // Add the character to the defender section 
                $("#nym-character").removeClass("enemy-character").addClass("defender-character");
                $("#defender-section").append(this);
            }
        }
    });

    $("#attack").on("click", function () {
        console.log("Attack selected");

        console.log("character = " + JSON.stringify(character));
        console.log("defender = " + JSON.stringify(defender));

        // User is ready to attack the defender
        if (characterSelected && defenderSelected && !gameOver) {
            // User attacks the defender and decreases the defender's health points
            defender.health = defender.health - character.attack;
            $(".defender-character").children(".health").html(defender.health);
            $("#game-message").html("<p>You attacked " + defender.name + " for " + character.attack + " damage.<p>");

            // User's attack power increases
            character.attack = character.attack + character.baseAttack;

            // If defender is still alive, they counter attack the user
            if (defender.health > 0) {
                character.health = character.health - defender.baseAttack;
                $(".chosen-character").children(".health").html(character.health);

                // Check if the user survives the attack
                if (character.health > 0) {
                    $("#game-message").append("<p>" + defender.name + " attacked you back for " + defender.baseAttack + " damage.</p>");
                } else {
                    gameOver = true;
                    $("#game-message").html("<p>You were defeated...</p><p>Play again</p>");
                    $("#restart").show();
                }
            } else {
                // Defender is defeated
                enemiesDefeated++;
                defenderSelected = false;
                $("#game-message").html("<p>You have defeated " + defender.name + ". Choose another enemy.</p>");
                $(".defender-character").hide();

                // Check if the user has won the game
                if (enemiesDefeated === 3) {
                    gameOver = true;
                    $("#game-message").html("<p>You have won the game!!!</p>");
                    $("#restart").show();
                }
            }
        } else if (!characterSelected && !gameOver) {
            $("#game-message").html("<p>You must first select your game character.</p>");
        } else if (!defenderSelected && !gameOver) {
            $("#game-message").html("<p>You must choose an enemy to fight.</p>");
        }

        console.log("character = " + JSON.stringify(character));
        console.log("defender = " + JSON.stringify(defender));
    });

    $("#restart").on("click", function () {
        console.log("Restart selected");

        resetGame();
    });

}); // Main routine